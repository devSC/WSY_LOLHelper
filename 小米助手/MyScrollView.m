...
{
    if (_urlStringArray != urlStringArray) {
        [_urlStringArray release];
        _urlStringArray = [urlStringArray retain];
        [self setScrollViewPropertyWithArray:_urlStringArray];
    }
}

- (void)setScrollViewPropertyWithArray: (NSArray *)array
{
    //移除scrollView上所有的imageView对象
    for (id view in _scrollView.subviews)
    {
        if ([view isKindOfClass:[MyScrollViewButton class]] == YES)
        {
    ...
    if ([array count] > 1)
    {
        ...
        _scrollView.contentSize = CGSizeMake(_scrollView.bounds.size.width*([array count] + 2), 0);
        
        CGRect rect = _scrollView.bounds;
        for (int i = 0; i < 3; i++)
        {
            rect.origin.x = _scrollView.bounds.size.width * i;
            _imageButton = [[[MyScrollViewButton alloc] initWithFrame:rect] autorelease];
            [_imageButton setUserInteractionEnabled:YES];
            ...
            NSDictionary *dic = [array objectAtIndex:imageIndex];
            //将数据由下层处理
            [_imageButton setDataDic:dic];
            _imageButton.tag = TAGOFFSET + i;
    ...
    else//就一张照片
    {
        ...
        _imageButton = [[[MyScrollViewButton alloc] initWithFrame:self.bounds] autorelease];
        [_imageButton setUserInteractionEnabled:YES];
        [_imageButton setDataDic:[array objectAtIndex:0]];
        [_scrollView addSubview:_imageButton];
        [_imageButton addTarget:self action:@selector(imageButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    }
...
- (void)imageButtonAction: (MyScrollViewButton *)button
{
    
    NSString *string = button.articleId;
    if (_myBlock) {
        _myBlock(string);
    }
}
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
     ...
     *--------------------------------------------------------------------------*/
    if (scrollView.contentOffset.x >= ([_urlStringArray count] + 1) * scrollView.bounds.size.width)
    {
        //把可重用的倒数第3页,放到第1页(页码排序是:0 1 2 3 ...3 2 1)
        [self thePageNumberReuseabled:([_urlStringArray count] - 1) andThePageNumberWillBePlaced:1];
        
        //把可重用的倒数第1页,放到第2页(页码排序是:0 1 2 3 ...3 2 1)
        [self thePageNumberReuseabled:([_urlStringArray count] + 1) andThePageNumberWillBePlaced:2];
        
        //把可重用的倒数第2页,放到第0页(页码排序是:0 1 2 3 ...3 2 1)
        [self thePageNumberReuseabled:[_urlStringArray count] andThePageNumberWillBePlaced:0];
    ...
    else if (scrollView.contentOffset.x <= 0.0f) //滚动到首页,要翻到最后
    {
        ...
        [self thePageNumberReuseabled:2 andThePageNumberWillBePlaced:[_urlStringArray count]];
        
        //把可重用的第1页,放到倒数第1页(页码排序是:0 1 2 3 ...3 2 1)
        [self thePageNumberReuseabled:1 andThePageNumberWillBePlaced:([_urlStringArray count] + 1)];
        
        //把可重用的第0页,放到倒数第3页(页码排序是:0 1 2 3 ...3 2 1)
        [self thePageNumberReuseabled:0 andThePageNumberWillBePlaced:([_urlStringArray count] - 1)];
        
        //把页面切换到倒数第二页
        [scrollView setTag:[_urlStringArray count]];
        scrollView.contentOffset = CGPointMake(scrollView.bounds.size.width * [_urlStringArray count], 0);
     ...
     *--------------------------------------------------------------*/
    if (pageNumber > scrollView.tag && pageNumber != ([_urlStringArray count] + 1))//往右翻页
    {
...
//        NSLog(@"%d",pageNumber);
    ...
    if (_pageControlEnabled == YES)
    {
        if (pageNumber == 0)
        {
            _pageControl.currentPage = [_urlStringArray count];
        }
        else if(pageNumber == 1 || pageNumber == [_urlStringArray count]+1)
        {
...
- (void)thePageNumberReuseabled:(NSInteger)srcPageNumber andThePageNumberWillBePlaced:(NSInteger)destPageNumber
{
    ...
    MyScrollViewButton *reusebledView = (MyScrollViewButton *)[_scrollView viewWithTag:(TAGOFFSET + srcPageNumber)];
    //把当前不显示的View放进可重用集合里
    ...
    MyScrollViewButton *rView = [_reusebledViews anyObject];
    //如果没有可以重用的View
    if (rView == nil)
    {
        rView = [[[MyScrollViewButton alloc] init] autorelease];
    ...
    {
        imageIndex = [_urlStringArray count] - 1;
    }
    else if(destPageNumber == [_urlStringArray count] + 1)//最后一页放首张照片
    {
        imageIndex = 0;
    }
    else
    ...
    //为view获得对应的图片路径
    NSDictionary *dic = [_urlStringArray objectAtIndex:imageIndex];
    //设置对应的图片
    [rView setDataDic:dic];;
}
