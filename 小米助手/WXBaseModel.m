...
{
    NSError *error = nil;
    NSString *contentString = [NSString stringWithContentsOfURL:[NSURL URLWithString:urlString] encoding:NSUTF8StringEncoding error:&error];
    if (error) {
        NSLog(@"hardLatestNewsCode error: %@", [error localizedDescription]);
    }
